...
# from model.based_transformer import Opinion_Transformer,Review_Base_Transformer
...
class FusionTransformerModel(nn.Module):
    def __init__(self,
        ...
        self.opinion_Transformer = Opinion_Transformer(input_text_embedding, aspect_type_embedding, label_embedding,
                                                       user_embedding, business_embedding,
                                                       passage_list_embedding, self.passage_EmbLayer, self.K_V_dim,
                                                       dim_feedforward, n_heads,
                                                       opinion_decoder_layers, padding_idx=padding_idx, device=device,
                                                       opt=opt)

        self.opinion_projection = nn.Linear(self.d_model,
                                            self.opinion_voc_size)

        self.review_Fusionsformer =Fusion_Transformer(self.passage_EmbLayer, pos_embedding
                                                       , out_text_embedding,
                                                       review_encoder_layers,
                                                       template_encoder_layers,
                                                       fusion_decoder_layers,
    ...
    def forward(self, input_text, aspect_type, label, user_id, business_id, passage_list, temp_pos, review_output,review_pos):
        ...
        # print(aspect_type.device)
        # print(label.device)
        # print(user_id.device)
        # print(business_id.device)
        # print(passage_list.device)
        # exit()
        for i in range(aspect_num):
            # print(i)
            opinion_outputs, opinion_self_attns, opinion_enc_attns, KL_loss_reconstrcut_i = self.opinion_Transformer(
                input_text[:, i, :], aspect_type[:, i], label[:, i], user_id, business_id, passage_list[:, i, :])
            opinion_outputs = self.opinion_projection(opinion_outputs)
            KL_loss_reconstrcut.append(KL_loss_reconstrcut_i)
        ...
        KL_loss_reconstrcut = sum(KL_loss_reconstrcut)
        ...
        return opinion_outputs_list, review_outputs, review_input, KL_loss_reconstrcut
    ...
    def opinion_generate(self, input_text, aspect_type, aspect_label, user, business, maxlen, bos_index, pad_index,
        ...
        memory_i = self.opinion_encode(input_text, aspect_type, aspect_label, user, business)
        if opt.use_keywords == "yes":
        ...
        for i in range(1, maxlen):
            decode_prob, _1, _2 = self.opinion_decode(tgt[:, :i], enc_inputs, memory_i)
    ...
    def review_generate_beam_search(self, opinion_input,segment_input, temp_input,out2pos, maxlen, bos_index, pad_index, opt,
        ...
        batch_size, input_seq_len = opinion_input.shape
        enc_outputs_OP, enc_self_attns_OP = self.review_Fusionsformer.opinion_enc(opinion_input,segment_input)
        enc_outputs_TP, enc_self_attns_TP = self.review_Fusionsformer.template_enc(temp_input)
        ...
        for i in range(1, maxlen):
            ...
            for j in range(len(active_beams)):
                ...
                decode_prob, _1, _2 = self.review_Fusionsformer.fusion_decoder(tgt[:, :i, active_beams[j]],tgt_pos,
    ...
    def generate_all(self, input_text, aspect_type, aspect_label, user, business, temp_pos, opinion_max_len,
                     review_max_len, bos_index, pad_index, opt, ):

        # 多条opinion
        # opinion_outputs_list
        # opinion_outputs_list 是opinion的真实预测值
        # review_bos_ 是通过opinion_outputs_list构建的review的输入
        #
        ...
        review_tgt = self.review_generate(review_input, temp_pos, maxlen, bos_index, pad_index, opt, )

        return opinion_outputs_list, review_tgt, review_input

    def generate_all_beamsearch(self, input_text, aspect_type, aspect_label, user, business, temp_pos,out2pos, opinion_max_len,
        ...
        batch_size, temp_pos_len = temp_pos.shape
        maxlen = min(temp_pos_len, review_max_len)
        review_tgt = self.review_generate_beam_search(review_input,segment_input, temp_pos,out2pos, maxlen, bos_index, pad_index, opt,
                                                      vocab_size,
...
def type_label_eval(id_tensor: torch.Tensor,
               label2word,
               ignore_pad: str = "<pad>",
               ignore_bos: str = "<bos>",
               ignore_eos: str = "<eos>",
    ...
    for sentence in sentences:
        denum_sentence = []
        for aspect_list in sentence:
            if isinstance(aspect_list,list):
                for label_id in aspect_list:
                    ...
                    if ignore_eos == label2word[label_id]:
                        ...
                        continue
                    denum_sentence.append(label2word[label_id])
                denum_sentence.append(",")
            else:
                if ignore_bos == label2word[aspect_list]:
                    continue
                if ignore_eos == label2word[aspect_list]:
                    continue
                elif ignore_pad == label2word[aspect_list]:
                    continue
                denum_sentence.append(label2word[aspect_list])
