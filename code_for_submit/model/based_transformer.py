...
class Fusion_Decoder(nn.Module):
    ...
    def __init__(self,review_embed,d_model,fusion_decoder_layers,d_ff,d_k,d_v,n_heads,padding_idx,device):
    ...
    def forward(self, dec_inputs, enc_inputs_OP, enc_outputs_OP,enc_inputs_TP, enc_outputs_TP):
...
class Review_Base_Transformer(nn.Module):
    def __init__(self,passage_EmbLayer,pos_embed ,review_embed,
        ...
        super(Review_Base_Transformer, self).__init__()
        passage_EmbLayer=passage_EmbLayer

        pos_EmbLayer = nn.Embedding.from_pretrained(torch.tensor(pos_embed,
                                                                          dtype=torch.float), freeze=False,
                                                             padding_idx=padding_idx)
        ...
        padding_idx  =0
        self.opinion_enc = Encoder(passage_EmbLayer,text_emb_dim,dim_feedforward,K_V_dim,K_V_dim,n_heads,review_encoder_layers, pad_index=padding_idx,device=device,segLayer=self.sge_EmbLayer)
        self.template_enc = Encoder(pos_EmbLayer,pos_dim,dim_feedforward,K_V_dim,K_V_dim,n_heads,template_encoder_layers, pad_index=padding_idx,device=device)

        print("----------------------------------this is Base Decoder----------------------------")

        self.fusion_decoder = Decoder(self.review_Emblayer,self.review_voc_size,text_emb_dim,dim_feedforward,K_V_dim,K_V_dim,n_heads,fusion_decoder_layers,padding_idx,device=device)
    ...
    def forward(self,enc_inputs_OP,enc_inputs_TP,dec_inputs,dec_inputs_pos,segment_input):
        ...
        #basetrnasormer
        # print(dec_inputs)
        # print(enc_outputs_OP)
        dec_outputs, ec_enc_attns_OP, ec_enc_attns_TP = self.fusion_decoder(dec_inputs, enc_inputs_OP, enc_outputs_OP)
        # print(dec_outputs)
