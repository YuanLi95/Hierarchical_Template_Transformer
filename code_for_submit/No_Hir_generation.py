...
from models import LabelSmoothingLoss, SumEvaluator, denumericalize,type_label,type_label_eval
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_epoch_pt', default="default_op2text_default_epoch-5.pt", type=str)
    ...
    output_filepath = os.path.join(basepath,
                                   "output",
                                   "{}_{}.csv".format(opt.model_name,opt.model_epoch_pt))
    ...
    model_weight = os.path.join(model_filepath,opt.model_epoch_pt)
    model_weight = model_weight.replace("\\","/")
    print(model_weight)
    ...
    for batch_idx, batch in enumerate(test_iterator):
        ...
        keyword = type_label_eval(batch["input_text"].data,absa_dataset.tokenizer_input.idx2word)

        label = type_label_eval(batch["aspect_label"].data, absa_dataset.tokenizer_aspect_label.idx2dependency)
