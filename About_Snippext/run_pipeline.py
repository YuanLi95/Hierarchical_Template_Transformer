...
def extract(review, config_list, models):
    ...
    for sent, tokens in zip(extractions, all_tokens):
        ...
        review['sentences'].append(" ".join(tokens))
    return review
...
def predict(input_fn, output_fn, config_list, models):
    ...
    with jsonlines.open(output_fn, mode='w') as writer:
        with open(input_fn,encoding="utf-8") as fin:
...
def initialize(checkpoint_path,
    ...
    checkpoints = dict([(task, os.path.join(checkpoint_path, \
                         'task_%s.pt' % task)) for task in tasks])
    print(checkpoints)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_fn", type=str, default='./yelp_data/yelp_aspect_datasets.csv')
    parser.add_argument("--output_fn", type=str, default='./aspect_datasets/datasets.jsonl')
    parser.add_argument("--use_gpu", dest="use_gpu", action="store_true")
    parser.add_argument("--fp16", dest="fp16", action="store_true")
    parser.add_argument("--checkpoint_path", type=str, default='checkpoints/')
    parser.add_argument("--lm", type=str, default='bert')
    parser.add_argument("--tasks", type=str, default='restaurant_tagging,pairing,restaurant_classification,restaurant_asc')
